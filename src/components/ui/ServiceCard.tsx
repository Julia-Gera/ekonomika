...
export default function ServiceCard({ number, title, href, icon, className = '' }: ServiceCardProps) {
  return (
    <Link
      href={href}
      className={`group flex h-[120px] flex-col justify-between bg-white p-5 transition-colors duration-200 hover:bg-[#8ba0be] ${className}`}
    >
      <div className="flex items-start justify-between">
        <span className="text-[14px] font-normal leading-[17px] text-[#030303] transition-colors group-hover:text-white">
          {number}
        </span>
        {icon ? (
          <Image src={icon} width={32} height={32} alt="" className="shrink-0" />
        ) : (
          <RedirectIcon className="text-[#9EA6B3] transition-colors group-hover:text-white" />
        )}
      </div>

      <div className="flex items-end justify-between">
        <p className="pr-4 text-[20px] font-normal leading-[1.2] text-[#0C2140] transition-colors group-hover:text-white">
          {title}
        </p>
        {icon && <RedirectIcon className="text-[#0C2140] transition-colors group-hover:text-white" />}
      </div>
    </Link>
  )
}
